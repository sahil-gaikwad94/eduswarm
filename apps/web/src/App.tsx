...
import { API, apiGet, apiPost, type Agent, type Page, type Pack, type Topic } from './lib/api';
...
export function App() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (String(e.message || '').includes('401')) { window.location.assign(`${API}/auth/google`); return; }
  ...
  async function start(t: Topic) {
    setSelected(t); setPage('lesson'); setError('');
    setJob({ status: 'queued', stage: 'Dean', message: 'The learning team is starting…' });
    let j: any;
    try {
      j = await apiPost('/api/jobs', { topicId: t.id });
    ...
    <div className="app-shell">
      ...
      <div className="workspace">
        ...
        <main className="main-content">
          ...
          {page === 'lesson' && (
            <Lesson topic={selected || topics[0]} pack={pack} job={job} onStart={start}
              onBack={() => setPage('plan')} onCards={() => setPage('flashcards')} onPractice={() => setPage('practice')} />
          )}
