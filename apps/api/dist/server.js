...
async function syncRuntimeJob(id) {
    for (let attempt = 0; attempt < 120; attempt += 1) {
        ...
        try {
            ...
            if (latest) {
                job.stage = latest.agent;
                job.message = latest.message;
                emit(id, { ...job, timestamp: latest.timestamp });
            }
            if (remote.status === 'completed') {
                job.status = 'completed';
                job.package = remote.package;
    ...
    if (job && job.status !== 'completed') {
        job.status = 'failed';
        job.message = 'The agent team timed out; please retry.';
        emit(id, { ...job, timestamp: new Date().toISOString() });
    }
}
