...
export type StoredReview = {
  id: string; ownerId: string; cardKey: string; topicId: string; question: string; answer: string;
  efactor: number; intervalDays: number; repetitions: number; nextDueAt: string; lastQuality: number; updatedAt: string;
};
export type StoredMock = Record<string, any> & { id: string; ownerId: string; status: string };
export type StoredActivity = {
  id: string; ownerId: string; kind: string; date: string; createdAt: string; xp?: number;
} & Record<string, any>;
...
export interface Store {
  ...
  saveAgentSession(session: StoredAgentSession): Promise<StoredAgentSession>;
  getAgentSession(ownerId: string, id: string): Promise<StoredAgentSession | null>;
  listAgentSessions(ownerId: string): Promise<StoredAgentSession[]>;
  saveMistake(mistake: StoredMistake): Promise<StoredMistake>;
  deleteMistake(ownerId: string, id: string): Promise<void>;
  listMistakes(ownerId: string): Promise<StoredMistake[]>;
  saveReview(review: StoredReview): Promise<StoredReview>;
  listReviews(ownerId: string): Promise<StoredReview[]>;
  saveMock(mock: StoredMock): Promise<StoredMock>;
  getMock(id: string): Promise<StoredMock | null>;
  listMocks(ownerId: string): Promise<StoredMock[]>;
  logActivity(activity: StoredActivity): Promise<void>;
  listActivities(ownerId: string, limit?: number): Promise<StoredActivity[]>;
...
class MemoryStore implements Store {
  ...
  private mocks = new Map<string, StoredMock>();
  ...
  async getUserByProvider(provider: string, subject: string) {
    return [...this.users.values()].find((u) => u.provider === provider && u.providerSubject === subject) || null;
  }
  async getJob(id: string) { return this.jobs.get(id) || null; }
  async saveJob(job: StoredJob) { this.jobs.set(job.id, job); return job; }
  async updateJob(id: string, patch: Partial<StoredJob>) {
  ...
  async getSession(id: string) {
    ...
    if (!s || s.expiresAt <= new Date()) { this.sessions.delete(id); return null; }
    return s;
  }
  ...
  async saveAgentSession(s: StoredAgentSession) { this.agentSessions.set(s.id, s); return s; }
  async getAgentSession(o: string, id: string) { const s = this.agentSessions.get(id); return s?.ownerId === o ? s : null; }
  async listAgentSessions(o: string) { return [...this.agentSessions.values()].filter((s) => s.ownerId === o); }
  async saveMistake(m: StoredMistake) { this.mistakes.set(`${m.ownerId}:${m.id}`, m); return m; }
  async deleteMistake(o: string, id: string) { this.mistakes.delete(`${o}:${id}`); }
  async listMistakes(o: string) { return [...this.mistakes.values()].filter((m) => m.ownerId === o); }
  async saveReview(r: StoredReview) { this.reviews.set(`${r.ownerId}:${r.cardKey}`, r); return r; }
  async listReviews(o: string) { return [...this.reviews.values()].filter((r) => r.ownerId === o); }
  async saveMock(m: StoredMock) { this.mocks.set(m.id, m); return m; }
  async getMock(id: string) { return this.mocks.get(id) || null; }
  async listMocks(o: string) {
    return [...this.mocks.values()].filter((m) => m.ownerId === o).sort((a, b) => String(b.createdAt).localeCompare(String(a.createdAt)));
  }
  async logActivity(a: StoredActivity) { this.activities.push(a); }
  async listActivities(o: string, limit = 500) {
    return this.activities.filter((a) => a.ownerId === o).slice(-limit);
  }
  async publish(id: string, event: unknown) { this.listeners.get(id)?.forEach((h) => h(event)); }
  async subscribe(id: string, h: (event: any) => void) {
    const set = this.listeners.get(id) || new Set();
    set.add(h);
    this.listeners.set(id, set);
    return async () => { set.delete(h); };
  }
}
...
class MongoRedisStore implements Store {
  ...
  constructor(private mongoUrl: string, private redisUrl: string) { this.mongo = new MongoClient(mongoUrl); }
  ...
  async connect() {
    ...
    await this.database.collection('reviews').createIndex({ ownerId: 1, nextDueAt: 1 });
    await this.database.collection('mocks').createIndex({ ownerId: 1, createdAt: -1 });
    await this.database.collection('activities').createIndex({ ownerId: 1, createdAt: -1 });
    await this.database.collection('activities').createIndex({ ownerId: 1, date: 1 });
  ...
  async upsertUser(u: StoredUser) { await this.database.collection<StoredUser>('users').replaceOne({ id: u.id }, u, { upsert: true }); return u; }
  async getUserByProvider(provider: string, providerSubject: string) {
    return await this.database.collection<StoredUser>('users').findOne({ provider, providerSubject }, { projection: { _id: 0 } });
  }
  async getJob(id: string) { return await this.database.collection<StoredJob>('jobs').findOne({ id }, { projection: { _id: 0 } }); }
  async saveJob(j: StoredJob) { await this.database.collection<StoredJob>('jobs').replaceOne({ id: j.id }, j, { upsert: true }); return j; }
  async updateJob(id: string, patch: Partial<StoredJob>) {
    return await this.database.collection<StoredJob>('jobs').findOneAndUpdate({ id }, { $set: patch }, { returnDocument: 'after', projection: { _id: 0 } }) || null;
  }
  async createSession(s: Session) { await this.database.collection<Session>('sessions').insertOne(s); }
  ...
  async saveAgentSession(s: StoredAgentSession) { await this.database.collection<StoredAgentSession>('agentSessions').replaceOne({ id: s.id }, s, { upsert: true }); return s; }
  async getAgentSession(o: string, id: string) { return await this.database.collection<StoredAgentSession>('agentSessions').findOne({ ownerId: o, id }, { projection: { _id: 0 } }); }
  async listAgentSessions(o: string) { return await this.database.collection<StoredAgentSession>('agentSessions').find({ ownerId: o }, { projection: { _id: 0 } }).toArray(); }
  async saveMistake(m: StoredMistake) { await this.database.collection<StoredMistake>('mistakes').replaceOne({ ownerId: m.ownerId, id: m.id }, m, { upsert: true }); return m; }
  async deleteMistake(o: string, id: string) { await this.database.collection<StoredMistake>('mistakes').deleteOne({ ownerId: o, id }); }
  async listMistakes(o: string) { return await this.database.collection<StoredMistake>('mistakes').find({ ownerId: o }, { projection: { _id: 0 } }).sort({ createdAt: -1 }).toArray(); }
  async saveReview(r: StoredReview) { await this.database.collection<StoredReview>('reviews').replaceOne({ ownerId: r.ownerId, cardKey: r.cardKey }, r, { upsert: true }); return r; }
  async listReviews(o: string) { return await this.database.collection<StoredReview>('reviews').find({ ownerId: o }, { projection: { _id: 0 } }).sort({ nextDueAt: 1 }).toArray(); }
  async saveMock(m: StoredMock) { await this.database.collection<StoredMock>('mocks').replaceOne({ id: m.id }, m, { upsert: true }); return m; }
  async getMock(id: string) { return await this.database.collection<StoredMock>('mocks').findOne({ id }, { projection: { _id: 0 } }); }
  async listMocks(o: string) { return await this.database.collection<StoredMock>('mocks').find({ ownerId: o }, { projection: { _id: 0 } }).sort({ createdAt: -1 }).toArray(); }
  async logActivity(a: StoredActivity) { await this.database.collection<StoredActivity>('activities').insertOne(a); }
  async listActivities(o: string, limit = 500) {
    return await this.database.collection<StoredActivity>('activities').find({ ownerId: o }, { projection: { _id: 0 } }).sort({ createdAt: -1 }).limit(limit).toArray();
  }
  async publish(id: string, event: unknown) { await this.publisher.publish(channel(id), JSON.stringify(event)); }
  async subscribe(id: string, h: (event: any) => void) {
    const sub = this.publisher.duplicate();
    await sub.connect();
    await sub.subscribe(channel(id), (p) => h(JSON.parse(p)));
    return async () => { await sub.unsubscribe(channel(id)); await sub.quit(); };
  }
}
