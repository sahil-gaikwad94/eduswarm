...
class MemoryStore implements Store {
  ...
  private contents = new Map<string, StoredContent>(); private progress = new Map<string, StoredProgress>(); private agentSessions = new Map<string, StoredAgentSession>(); private listeners = new Map<string, Set<(event: any) => void>>();
...
class MongoRedisStore implements Store {
  ...
  async connect() { await this.mongo.connect(); this.database = this.mongo.db(); await this.database.collection('sessions').createIndex({ expiresAt: 1 }, { expireAfterSeconds: 0 }); await this.database.collection('users').createIndex({ provider: 1, providerSubject: 1 }, { unique: true, sparse: true }); await this.database.collection('contents').createIndex({ ownerId: 1, topicId: 1 }, { unique: true }); await this.database.collection('progress').createIndex({ ownerId: 1, topicId: 1 }, { unique: true }); this.publisher = createClient({ url: this.redisUrl }); await this.publisher.connect(); }
