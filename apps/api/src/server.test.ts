...
test('job creation fails closed when the agent runtime is unavailable', async () => { const created = await request('/api/jobs', { method: 'POST', body: { topicId: 'algo-complexity' } }); assert.equal(created.status, 202); await new Promise((resolve) => setTimeout(resolve, 100)); const result = await request(`/api/jobs/${created.body.id}`); assert.equal(result.body.status, 'failed'); const content = await request(`/api/content/${created.body.id}`); assert.equal(content.status, 404); });
