...
test('onboarding creates an independent goal', async () => { const result = await request('/api/onboarding', { method: 'POST', body: { name: 'Ada', goal: 'gate-cs', dailyMinutes: 30 } }); assert.equal(result.status, 201); assert.equal(result.body.goals.length, 1); assert.equal(result.body.dailyMinutes, 30); });
	...
	test('curriculum includes the full tutorial path and module metadata', async () => { const result = await request('/api/curriculum/gate-cs'); assert.equal(result.body.topics.length, 12); assert.equal(result.body.topics.some((topic: any) => topic.title === 'Dynamic Programming' && topic.module === 'Advanced strategies'), true); });
