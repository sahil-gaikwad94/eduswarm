...
app.get('/api/jobs/:id', async (req: Request, res: Response) => { const user = await requireUser(req, res); if (!user) return; const job = await store.getJob(req.params.id); if (!job || job.ownerId !== user.id) return res.status(404).json({ error: 'Job not found' }); res.json(job); });
...
app.get('/api/content/:jobId', async (req: Request, res: Response) => { const user = await requireUser(req, res); if (!user) return; const job = await store.getJob(req.params.jobId); if (!job || job.ownerId !== user.id || !job.package) return res.status(404).json({ error: 'Verified package is not ready' }); res.json(job.package); });
